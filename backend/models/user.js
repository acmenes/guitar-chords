...
class User {
  ...
  static async addChordToList(username, chord_fullname) {
    const checkChord = await db.query(
      `SELECT chord_fullname 
                                            FROM chords 
                                            WHERE chord_fullname=$1`,
      [chord_fullname]
    );
    ...
    await db.query(
      `INSERT INTO user_chords (username, chord_fullname) 
                            VALUES ($1, $2)`,
      [username, chord_fullname]
    );
  }

  static async removeChordFromChordsList(username, chord_fullname) {
    const checkChord = await db.query(
      `SELECT chord_fullname 
                                            FROM chords 
                                            WHERE chord_fullname=$1`,
    ...
    await db.query(
      `DELETE FROM user_chords where username=$1 and chord_fullname=$2`,
      [username, chord_fullname]
    );
  }
  ...
  static async editUserChord(username, chord_fullname, chordDone) {
    const result = await db.query(
      `UPDATE user_chords
                                        SET done = ${chordDone}
                                        WHERE username = $1 AND chord_fullname = $2
                                        RETURNING chord_fullname, done`,
      [username, chord_fullname]
    );
