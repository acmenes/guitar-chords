...
router.post(
  "/:username/chords/:chord_fullname",
  ensureCorrectUserOrAdmin,
  async function (req, res, next) {
    try {
...
router.delete(
  ...
  async function (req, res, next) {
    try {
      const chord = req.params.chord_fullname;
      await User.removeChordFromChordsList(req.params.username, chord);
      return res.json({ removed: chord });
    } catch (err) {
      return next(err);
    }
  }
...
router.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      const chord = req.params.chord_fullname;
      const chordDone = req.body.chordDone;
      await User.editUserChord(username, chord, chordDone);
      return res.json({ updated: chord, status: chordDone });
    } catch (err) {
      return next(err);
    }
  }
